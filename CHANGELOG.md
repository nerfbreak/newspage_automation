...
- **Adaptasi UI Newspage Baru (Kritis)**: Sistem Newspage ternyata baru saja melakukan pembaruan UI secara diam-diam! Mereka menghapus total fitur pencarian Interface ID menggunakan sistem Popup Search (tombol *kaca pembesar* / SelectButton), dan menggantinya menjadi kolom *text input* langsung. Bot kini sudah diprogram ulang untuk menuliskan ID secara langsung dan memicu *AutoPostBack* via `Tab` tanpa perlu mencari tombol popup yang sudah hilang. Hal ini memulihkan seluruh fungsi ekstraksi (Clearance, Sales, dan Promo Comparison) yang sebelumnya lumpuh.
