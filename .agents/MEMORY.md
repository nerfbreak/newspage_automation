...
- **2026-06-30**: Added "Interactive Inspector (ALT+Click)" mode to Element Crawler (`pages/7_element_crawler.py` and `playwright_engine.py`) allowing manual interactive DOM extraction in a live Chromium window.
- **2026-06-30**: Installed `spec-kit` (specify-cli v0.11.10) via `uv tool install`. Initialized Spec-Driven Development workflow with `agy` (Antigravity) integration. Ratified project constitution v1.0.0 at `.specify/memory/constitution.md` encoding all existing rules: Feature Freeze, Security-First, Selector Integrity, UI Consistency, and Session Logging principles.
- **2026-06-30**: **Full Page Load Wait — Feature 001 (Dama Unlocked):** Added `_wait_for_page_ready(page, timeout_ms, ui_log, context)` helper function to `playwright_engine.py` (additive only, no business logic changed). Uses `page.wait_for_load_state("networkidle")` capped at 30s with graceful fallback. Injected at ALL navigation checkpoints and submit actions across: `_navigate_to_import_export`, `_navigate_to_stock_adjustment`, `_dispatch_extraction_job`, `_dispatch_sales_job`, `_dispatch_promotion_job`, `_inject_adjustment_row` (Add to grid), `_inject_manual_adjustment_row` (Add to grid), and all final Save/Confirm/Logout actions in `run_execution` and `run_execution_manual`. All existing `page.wait_for_timeout()` calls preserved as-is. Full spec artifacts in `specs/001-full-page-load-wait/`.
