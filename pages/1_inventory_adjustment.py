...
main_left, main_right = st.columns([0.7, 0.3], gap="large")

if "Auto Compare" in adj_mode:
    # ---------------- LEFT COLUMN (70%) ----------------
    with main_left:
        st.markdown("<div class='header-wrapper-left'><span class='section-header-underline'>DATA CONFIGURATION</span></div>", unsafe_allow_html=True)
        col_dist, col_file = st.columns(2)
        ...
        with col_dist:
            with st.container(border=True):
                ...
                # Hidden label variable for Right Column
        ...
        if st.session_state.np_df is not None:
            ...
            if st.button("Clear Extracted Data", width="stretch", icon=":material/delete:"):
                st.session_state.np_df = None
                st.rerun()

        # Data Comparison Logic placeholders
        file1 = None
        ...
        if st.session_state.reconcile_summary is not None and st.session_state.reconcile_result is not None:
            st.markdown("<div class='header-wrapper-left'><span class='section-header-underline'>STOCK REVIEW</span></div>", unsafe_allow_html=True)
            ...
            st.markdown("<div class='header-wrapper-left'><span class='section-header-underline'>ADJUSTMENT SKU LIST</span></div>", unsafe_allow_html=True)
            table_placeholder = st.empty(); table_placeholder.dataframe(df_view, width="stretch", hide_index=True)


    # ---------------- RIGHT COLUMN (30%) ----------------
    with main_right:
        st.markdown("<div class='header-wrapper-left'><span class='section-header-underline'>ACTION CENTER</span></div>", unsafe_allow_html=True)
        ...
        start_adj_btn = False
        if np_source_ready and file2:
            start_adj_btn = st.button("Start Adjustment", type="primary", width="stretch", icon=":material/play_arrow:")
        ...
        if st.session_state.reconcile_summary is not None and st.session_state.reconcile_result is not None:
            ...
            st.markdown(f'''<div class="metric-box-match" style="margin-top: 16px;"><div class="metric-label">Match</div><div class="metric-value">{match_count}</div></div>''', unsafe_allow_html=True)
            st.markdown(f'''<div class="metric-box-mismatch"><div class="metric-label">Stock difference</div><div class="metric-value">{mismatch_count}</div></div>''', unsafe_allow_html=True)
            
            execute_btn = btn_placeholder.button("EXECUTE", type="primary", width="stretch", icon=":material/play_arrow:")
    ...
    with main_left:
        st.markdown("<div class='header-wrapper-left'><span class='section-header-underline'>MANUAL ENTRY SETUP</span></div>", unsafe_allow_html=True)
        list_dist = database.get_distributor_list(supabase)
        url_d = st.query_params.get("d", None)
        ...
        with st.container(border=True):
            st.markdown("<div class='header-wrapper-left'><span class='section-header-underline'>FIELD SKU INPUT</span></div>", unsafe_allow_html=True)
            st.markdown("""
    ...
    with main_right:
        ...
        exec_manual_btn = btn_placeholder.button("EXECUTE MANUAL ADJUSTMENT", type="primary", width="stretch", icon=":material/play_arrow:")
